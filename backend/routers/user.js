...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json(utils.createSuccess({ name: newUser.name, email: newUser.email }));
    } catch (error) {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json(utils.createSuccess({ id: user.id, name: user.name, email, role: user.role, token  }));
    } catch (error) {
